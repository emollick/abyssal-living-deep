...
import { HABITATS, GENERATOR_DEFAULTS, habitatFor, cameraPose, constrainSwimmer, parseSeed } from './WorldMath.js';
...
export class Expedition {
  constructor(app) {
    ...
    this.enterDive();
    if(app.params.get('surface')==='1')this.surface();
  ...
  buildUI() {
    ...
    root.innerHTML=`
      ...
      <header class="dive-header">
        ...
        <nav class="dive-actions" aria-label="Explorer tools">
          <button class="dive-action" id="dive-surface">${icon('up')}<span>Surface</span></button>
      ...
      <div class="dive-depth"><strong id="dive-depth-value">13.0</strong> m<small>BELOW THE SURFACE</small><span class="depth-rule"></span></div>
      ...
      <aside class="dive-lab" id="dive-lab" aria-label="Procedural world lab" hidden>
        <h2>Grow an ocean.</h2><p class="dive-lab-intro">Every reef, leaf and animal is made here, from a seed. Change the recipe and make it yours.</p>
        ...
        <p class="lab-note" id="weather-depth-note">Sunlight, cloud cover and wind drive the light and motion below. Visit the surface to shape the waves and trigger disasters.</p>
        <h3>VIEW</h3><label for="dive-quality">Rendering quality</label><select id="dive-quality"><option value="auto">Automatic</option><option value="high">High</option><option value="ultra">Ultra</option><option value="medium">Medium</option><option value="low">Low</option><option value="potato">Lightest</option></select>
    ...
    guide.innerHTML=`<h2>A sea of your making.</h2><p>Let the camera drift, or take a swim. Four dive sites are starting recipes for an imagined ocean; the world lab changes the actual generated landscape and life.</p><dl><dt>W A S D</dt><dd>Swim in the direction you look</dd><dt>Drag</dt><dd>Look around; wheel to zoom</dd><dt>Q / E</dt><dd>Swim down / up</dd><dt>Shift</dt><dd>Move faster</dd><dt>1 – 4</dt><dd>Visit a dive site</dd><dt>G / R</dt><dd>World lab / new seed</dd><dt>F / P / H</dt><dd>Swim or drift / pause / hide controls</dd><dt>L</dt><dd>Toggle your dive light</dd></dl><p>On touchscreens, drag to look and use the swimming buttons. Desktop graphics are recommended; lower the quality if your device struggles.</p><p>All geometry, textures, light and motion are procedural. This is an artistic simulation, not an ecological or oceanographic model.</p><p>Built on <a href="https://github.com/Token-Gremlin/natural-disasters" target="_blank" rel="noopener">ABYSSAL by Token-Gremlin</a>, preserving its FFT ocean, atmosphere and extreme weather. MIT licensed.</p><button id="close-dive-guide">Back to the water</button>`;
    ...
    const back=document.createElement('button');back.className='surface-return';back.textContent='↓ Explore the living deep';back.onclick=()=>this.enterDive();document.body.appendChild(back);
    $('dive-surface').onclick=()=>this.surface();$('dive-lab-toggle').onclick=()=>this.toggleLab();
    ...
    $('new-seed').onclick=()=>this.newSeed();$('reset-world').onclick=()=>{this.world.settings={...GENERATOR_DEFAULTS};this.regenerate(habitatFor(this.world.habitat.id).seed);this.syncDials();};
    $('share-world').onclick=()=>this.share();$('show-dive-ui').onclick=()=>document.body.classList.remove('dive-clean');
  ...
  addDial(parent,key,label,min,max,step,live,weather=false) {
    ...
    const fmt=v=>key==='sunElevation'?`${v.toFixed(0)}°`:key==='windSpeed'?`${v.toFixed(1)} m/s`:key==='swellHs'?`${v.toFixed(1)} m`:weather?`${Math.round(v*100)}%`:`${v.toFixed(2)}×`;
    const apply=()=>{
      const value=+input.value;out.textContent=fmt(value);
      if(weather){this.app.weather.set({[key]:key==='sunElevation'?value*Math.PI/180:value});this.weatherMode='custom';this.syncWeatherButtons();}
      else this.world.settings[key]=value;
      if(live)this.writeURL();
    };
    input.addEventListener('input',apply);
    const commit=()=>{apply();if(!live)this.regenerate(this.world.habitat.seed,false);};
    ...
    this.dials[key]={input,out,fmt,weather};
  }
  ...
  enterDive() {
    if(this.hasEntered&&!this.active){this.weatherMode='custom';this.weatherBaseMode='day';}
    this.app.sandbox.setActive(false);this.app.director.enabled=false;this.app.director.clearEvents();
    ...
    U.uLamp.value=this.world.habitat.id==='deep'?0.9:0;
    this.hasEntered=true;this.syncLabels();this.syncDials();this.syncWeatherButtons();this.writeURL();
  }

  surface() {
    this.pendingGeneration=(this.pendingGeneration||0)+1;$('dive-lab').setAttribute('aria-busy','false');
    this.active=false;this.toggleLab(false);document.body.classList.remove('diving','dive-clean','dive-swimming');
    this.app.cine.diveController=null;this.app.cine.freeSpeed=40;
    this.app.camera.position.set(0,16,45);this.app.cine._smoothLook.set(0,8,-200);this.app.camera.lookAt(0,8,-200);
    this.app.sandbox._entered=true;this.app.setSandbox(true);this.app.cine.setZoom(55);
    Object.assign(this.app.post.settings,this.surfacePost);this.app.post.reset=true;this.writeURL();
  }

  visit(id) {
    if(id===this.world.habitat.id)return;
    this.pendingGeneration=(this.pendingGeneration||0)+1;$('dive-lab').setAttribute('aria-busy','false');
    this.world.generate(id,habitatFor(id).seed,this.world.settings);this.started=this.app.time;
    this.resetView();this.app.cine.setFree(false);this.setSwim(false);
    U.uLamp.value=id==='deep'?0.9:0;this.syncLabels();this.syncDials();this.writeURL();
  }
  ...
  async regenerate(seed,reset=true) {
    ...
    try {
      ...
      if(reset){this.started=this.app.time;this.resetView();}
      this.syncLabels();this.syncDials();this.writeURL();this.toast(`World ${this.world.habitat.seed} generated`);
  ...
  resetView() {
    const h=this.world.habitat,c=this.app.cine,cam=this.app.camera;
  ...
  updateCamera(dt,time) {
    const pose=cameraPose(this.world.habitat,time-this.started),c=this.app.cine,cam=this.app.camera;
    cam.position.fromArray(pose.eye);cam.lookAt(...pose.look);c._smoothLook.fromArray(pose.look);c._smoothPos.copy(cam.position);
    cam.fov=56;cam.updateProjectionMatrix();c.focusDistance=cam.position.distanceTo(c._smoothLook);
  }

  constrain(position) { constrainSwimmer(position,this.world.habitat); }

  setSwim(on) {
    const c=this.app.cine;
    if(c.free!==on)c.setFree(on);
    if(!on)this.started=this.app.time;
  ...
  setWeather(mode,immediate=false) {
    const presets={day:{...CONDITIONS.clear.w,sunElevation:0.91,sunAzimuth:2.3},dusk:CONDITIONS.golden.w,storm:CONDITIONS.storm.w,night:{...CONDITIONS.clear.w,sunElevation:-0.17,sunIntensity:9,cloudCoverage:0.16,starIntensity:1}};
  ...
  syncDials() {
    for(const [key,d] of Object.entries(this.dials)) {
      ...
      if(key==='sunElevation')v*=180/Math.PI;
      d.input.value=v;d.out.textContent=d.fmt(v);
    }
    $('world-seed').value=this.world.habitat.seed;
    $('dial-height').disabled=this.world.habitat.id!=='kelp';
    if(this.world.habitat.id!=='kelp')$('out-height').textContent='Kelp site only';
    $('dial-height').title=this.world.habitat.id==='kelp'?'Height of generated kelp':'Visit the kelp forest to change canopy height';
  }
  ...
  syncLabels() {
    ...
    $('dive-status-text').textContent=`LIVING WORLD · SEED ${h.seed}`;
    $('world-stats').textContent=`SEED ${h.seed} · ${this.world.stats.fish.toLocaleString()} FISH · ${this.world.stats.animals} LARGE ANIMALS`;
    $('world-stats').dataset.generation=this.world.generation;$('world-stats').dataset.vertices=this.world.stats.vertices;
    $('weather-depth-note').textContent=h.id==='deep'?'Sunlight does not reach this depth. A dive light reveals the mineral chimneys; bioluminescence lights the living garden.':'Sunlight, cloud cover and wind drive the light and motion below. Visit the surface to shape the waves and trigger disasters.';
    $('world-seed').value=h.seed;
  }
  ...
  lamp() { U.uLamp.value=U.uLamp.value>0?0:1; }

  updateUI() {
    if(!this.active)return;
    const a=this.app;
    a.post.settings.exposureBias=this.world.habitat.id==='deep'?0.78:(1-U.uDiveNight.value*0.45)*(1-a.weather.state.storm*0.26);
    a.post.settings.fixedExposure=this.world.habitat.id==='deep'?1.7:0;
    a.post.settings.tonemap=1;
    a.post.settings.contrast=this.world.habitat.id==='deep'?1.0:1.04;
    a.post.settings.wetLens=0;a.post.settings.dof=false;a.post.settings.motionBlur=false;
    if(performance.now()-this.lastReadout<150)return;this.lastReadout=performance.now();
    $('dive-depth-value').textContent=Math.max(0,-a.camera.position.y).toFixed(1);
    $('dive-depth-value').dataset.depth=(-a.camera.position.y).toFixed(3);
    $('dive-status-text').textContent=`${a.paused?'PAUSED':a.cine.free?'FREE SWIM':'SLOW DRIFT'} · SEED ${this.world.habitat.seed}`;
  ...
  writeURL() {
    ...
    const p=new URLSearchParams();p.set('site',this.world.habitat.id);p.set('seed',this.world.habitat.seed);
    ...
    if(!this.active)p.set('surface','1');
  ...
  photograph() {
    this.app.render(0);const a=document.createElement('a');a.download=`abyssal-${this.world.habitat.id}-${this.world.habitat.seed}.png`;a.href=this.app.canvas.toDataURL('image/png');a.click();this.toast('Photograph saved without the controls.');
  }
